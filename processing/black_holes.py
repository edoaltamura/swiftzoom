...
class BlackHoles:
    ...
    def find_central(self, search_radius: unyt.unyt_quantity = 100 * unyt.kpc) -> int:
        ...
        isnot_central_largest = self.group_zoom.black_holes.subgrid_masses[index_in_sw_data_central] < largest_bh_mass
        if isnot_central_largest:
            print(
                (
                    f"The central BH found is not the largest in the box:\n"
                    
                    f"\tCentral BH: mass = {self.group_zoom.black_holes.subgrid_masses[index_in_sw_data_central].to('Msun'):.5E} "
                    f"ID = {self.group_zoom.black_holes.particle_ids[index_in_sw_data_central].value:d} "
                    f"Distance from the CoP = {self.group_zoom.black_holes.radial_distances[index_in_sw_data_central].to('kpc'):.2f} "
                    ...
                    f"Distance from the CoP = {self.group_zoom.black_holes.radial_distances[largest_bh_index].to('kpc'):.2f} "
